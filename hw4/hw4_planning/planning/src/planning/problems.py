...
class R2Problem(PlanarProblem):
    def compute_distance(self, q1, q2):
        ...
        ### BEGIN QUESTION 1.1 #####################
        distance=np.linalg.norm(q2_2d-q1_2d, axis=1)
        return distance
        ### END QUESTION 1.1 #######################
...
class SE2Problem(PlanarProblem):
    ...
    def compute_distance(self, q1, q2):
        ...
        path_length=dubins.path_length(q1_2d, q2_2d, self.curvature)
        return path_length
        ### END QUESTION 4 #######################
...
class JointSpace(object):
    ...
    def compute_distance_rrt(self, start_config, goal_config):
        ...
        '''
        joint_v=np.array([1.0,1.2,0.8,0.75,1.05,0.9])
        time=np.zeros(6)
        for i in range(6):
           time[i]=np.abs(((start_config[:,i]-goal_config[:,i])+np.pi)%(2*np.pi)-np.pi)
           time[i]=time[i]/joint_v[i]
        time=np.max(time)
        return time
        ### END QUESTION 6 #######################
    ...
    def cost_to_go(self, config, goal_config):
        ...
        '''
        #heuristic=np.sum(self.compute_distance(config,goal_config))
        #heuristic=np.sum(np.abs((config_2d+np.pi)%(2*np.pi)-(goal_config_2d+np.pi)%(2*np.pi)))
        heuristic = np.max(np.abs((config_2d - goal_config_2d + np.pi) % (2*np.pi) - np.pi))
        return heuristic
        ### END QUESTION 5 #######################
