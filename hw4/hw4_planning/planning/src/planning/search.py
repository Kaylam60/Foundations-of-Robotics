...
class ASTARPlanner(object):
    ...
    def Plan(self, start, goal):
        ...
        while len(queue) > 0:
            ...
            for neighbor, w in self.rm.graph[entry.node].items():
                ...
                ### BEGIN QUESTION 1.2 #####################
                cost_to_come_neighbor=entry.cost_to_come + weight
                fv_neighbor=cost_to_come_neighbor+ h
                queue.push(QueueEntry(fv_neighbor, next(c), neighbor, entry.node, cost_to_come_neighbor))
                ### END QUESTION 1.2 #####################
    ...
    def extract_path(self, parents, goal):
        ...
        vpath=[goal]
        curnode=goal
        while parents[curnode]!=NULL:
            vpath.insert(0, parents[curnode])
    ...
    def Plan(self, start_config, goal_config, epsilon=0.001):
        ...
        for _ in range(self.max_iter):
            ...
            for i in range(xs_rand.shape[0]):
                ...
                '''
                x_nearid, x_near=self.tree.GetNearestVertex(x_rand)  
                x_new=self.extend(x_near, x_rand)
                if x_new is not None:
                    x_newid=self.tree.AddVertex(x_new)
                    self.tree.AddEdge(x_nearid, x_newid ) 
                    new_eid.append(x_newid)
