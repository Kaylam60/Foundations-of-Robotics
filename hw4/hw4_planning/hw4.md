...
When you are done, run the `python3 test/tradeoff.py` to test your maps. We test that A* prevails over RRT by verifying that A* finds a path while RRT takes more than 10 seconds to find a path. We test that RRT prevails over A* by verifying that RRT finds a path within 10 seconds while A* finds no path.
...
1 point per A* and RRT screenshot (2 max) <br>
1.5 points per correct explanation of tradeoffs (3 max)
</details>
...
### Q6: Design distance function for RRT (20 points)
