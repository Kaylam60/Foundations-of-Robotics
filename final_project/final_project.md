# Final Project
## Before You Start
This final project is dependent on HW1 - HW5. Please start this homework after you finish HW4 and HW5. You may replace HW1-3 with the previously released solutions.
...
The AGV uses a particle filter for localization, and there's a planner to plan the path. There are 3 rooms in the hospital, i.e. room A, room B, and room C. In room A, there is a medicine that can extend her lifetime by 50 seconds. In room B, there is a button to call the doctor and let them come earlier by 20 seconds, which means they will come in 180 seconds. In room C, there is a medicine that can add 20 seconds to the patient's life. The AGV can get medicine if it reaches the center of the rooms. 
...
3. Implement the `is_goal_reached` in `scripts/security_bot.py` to check if the AGV reaches the goal pose.
4. Determine the sequence of room for you to grab medicine in `waypoints.json`. Each room is represented by a tuple with three elements, `[x, y, heading_angle]`. Feel free to play with the heading angles, or add waypoints in the middle for a better path. The `[x,y]` positions for the rooms and Cathy are: 
- C: [14.4, 3.69]
- B: [1.8, 5.4]
- A: [3.19, 1.1]
- Cathy: [9.7, 1.2]
