...
def compute_position_in_frame(p, frame):
    ...
    R_frame=np.array([[np.cos(frame[2]),-np.sin(frame[2])],
                     [np.sin(frame[2]),np.cos(frame[2])]])
    d_position=np.array(p[:2])-np.array(frame[:2])
    error=np.dot(R_frame, d_position)
    return error
    # END SOLUTION
...
class BaseController(object):
    ...
    def get_reference_index(self, pose, path_xytv, distance_lookahead):
        ...
        with self.path_condition:
            ...
            deltas=path_xytv[:,:2]-np.array(pose[:2])
            distances=np.linalg.norm(deltas, axis=1)
            closest_index=np.argmin(distances)
            for i in range(closest_index, len(path_xytv)):
                if np.sqrt(distances[i]) >= distance_lookahead:
                    return i
            # END SOLUTION
