...
class ModelPredictiveController(BaseController):
    ...
    def sample_controls(self):
        ...
        alpha=np.linspace(self.min_alpha,self.max_alpha,self.K)
        controls[:,:,1]=alpha[:,np.newaxis]
        # END SOLUTION
        return controls
    ...
    def get_rollout(self, pose, controls, dt=0.1):
        ...
        for t in range(1,self.T):
            curr_controls=controls[:,t-1,:]
            delta=self.motion_model.compute_changes(rollouts[:,t-1,:],curr_controls,dt)
            rollouts[:,t,:]=rollouts[:,t-1,:]+delta
        # END SOLUTION
        return rollouts
    ...
    def compute_distance_cost(self, rollouts, reference_xyt):
        ...
        distances=np.linalg.norm(rollouts[:,-1,:2]-reference_xyt[:2],axis=1)
        costs=self.error_w*distances
        return costs
        # END SOLUTION
    ...
    def compute_collision_cost(self, rollouts, _):
        ...
        # BEGIN SOLUTION "QUESTION 4.3" ALT="raise NotImplementedError"
        costs=self.collision_w*rollout_collisions.sum(axis=1)
        return costs
        # END SOLUTION
    ...
    def get_control(self, pose, reference_xytv, _):
        ...
        control=self.sampled_controls[min_cost_i,0,:]
        return control
        # END SOLUTION
