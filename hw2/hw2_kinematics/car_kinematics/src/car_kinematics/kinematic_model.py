...
class KinematicCarMotionModel:
    ...
    def compute_changes(self, states, controls, dt, alpha_threshold=1e-2):
        ...
        v=controls[:, 0]
        alpha=controls[:, 1]
        zero_steer=np.abs(alpha)<alpha_threshold
        dtheta=(v/L)*np.tan(alpha)*dt

      
        dx=np.where(zero_steer,v*np.cos(theta)*dt, (L/np.tan(alpha))*(np.sin(theta+dtheta)-np.sin(theta)))
        dy=np.where(zero_steer,v*np.sin(theta)*dt,(L/np.tan(alpha))*(-np.cos(theta+dtheta)+np.cos(theta)))
        dtheta=np.where(zero_steer,0, dtheta)
    ...
    def apply_deterministic_motion_model(self, states, vel, alpha, dt):
        ...
        controls=np.zeros((n_particles,2))
        controls[:,0]=vel
        controls[:,1]=alpha
        compute=self.compute_changes(states, controls, dt)
        states+=compute
        states[:,2]=np.arctan2(np.sin(states[:, 2]), np.cos(states[:,2]))
        states[:,2]=np.where(states[:,2]>np.pi, states[:,2]-2*np.pi, states[:,2])
        states[:,2]=np.where(states[:,2]<=-np.pi, states[:,2]+2*np.pi, states[:,2])
        # END SOLUTION
    ...
    def apply_motion_model(self, states, vel, alpha, dt):
        ...
        controls=np.zeros((n_particles,2))
        controls[:,0]=np.random.normal(vel, self.vel_std,n_particles )
        controls[:,1]=np.random.normal(alpha, self.alpha_std,n_particles )
