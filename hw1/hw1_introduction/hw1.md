...
```
  > Run the above configuration with the figure_8.txt and crown.txt plan files. For both plans, save the figures produced by pose_listener for submission: locations.png and norms.png. *Important:* Make sure to restart the simulator before generating figures for each of the two plan files!

  > Optional: Let your artistic side shine and create your own plan file! If you do, please submit the plan file (and any code you may have used to generate it), as well as the locations.png producted by pose_listener. *Plan file format*: The first line of your plan file should specify the initial pose of the car (x,y, heading angle), while every line thereafter should specify a (forward velocity, angular velocity) pair. Additionally, make sure that there are no empty lines in your plan file (otherwise you may get garbage output).
...
To include figures in your submission, add the images to the same folder containing README.md, then include the image in your README with the following command: `![The title of your image](imageName.png)`.
