...
class ParticleInitializer:
    ...
    def reset_click_pose(self, msg, particles, weights):
        ...
        init_x=msg.position.x
        init_y=msg.position.y
        init_theta=utils.quaternion_to_angle(msg.orientation)

        x_samp=self.rng.normal(loc=init_x, scale=self.x_std, size=n_particles)
        y_samp=self.rng.normal(loc=init_y, scale=self.y_std, size=n_particles)
        theta_samp=self.rng.normal(loc=init_theta, scale=self.theta_std, size=n_particles)

        particles[:, 0]=x_samp
        particles[:, 1]=y_samp
        particles[:, 2]=theta_samp
