...
## Setup
Download skeleton code at https://github.coecis.cornell.edu/bd324/cs4750_hw3
```
# Open a terminal
$ cd ~/Downloads
...
Hint: When computing noise, make sure to use the rng object that comes with each object instead of calling np.random on your own. This is because the test cases are coded to read with the specific seed that is set in each constructor. You can call distributions from the rng object.
...
```
Hint: When initializing the first value, we can use both random and uniform. But in the project, please use uniform as the test cases expect that.
...
**n.b. In lecture notes, we provided pseudocode for Sequential-Importance-Sampling and the Particle Filter. As mentioned in the notes, one key difference is how the weights are calculated. It is important to note, while the pseudocode for the particle filter does not require normalized weights, the sampler may. Pay close attention to how the low-variance resampler functions to determine this.
...
Note: When you run the above command, a plot will pop up. Please close the plot, or you will receive a TimeOut error message and will not pass the test
...
**Please include your figure in your submission**. Place this figure in a new folder named `figures` in your submission so that its relative path inside the submission is: `/hw3_state_estimation/figures/position_error.png`
...
Download the zip file from https://github.com/empriselab/range_libc.git. 
...
## Submission
Zip your homework code and submit to Gradescope. Due date: Nov. 1st. 
